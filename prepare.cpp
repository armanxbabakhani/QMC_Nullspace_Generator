...
pair<int, int> Cycle_minimize(vector<vector<int>>& null_eigs , int min_cyc_length){
    pair<int, int> cycdata;
    vector<int> null_eigs_min_ind , null_eigs_high_ind;
    int min_cyc = 10000 , sum_i , nullsize = null_eigs.size();
    vector<int> null_n(nullsize);
    bool min_cyc_searching = true;
    for(int i = 0; i < nullsize; i++){
        sum_i = Int_sum(null_eigs[i]);
        null_n[i] = sum_i;
        if(sum_i == min_cyc_length){
            null_eigs_min_ind.push_back(i);
            min_cyc = sum_i;
            min_cyc_searching = false;
        }
        else{
    ...
    for(int k = 0; k < high_size; k++){
        ...
        for(int l = 0; l < nullsize; l++){
            if(l != null_eigs_high_ind[k]){
                ...
                if(min_cyc_searching & (low_k < min_cyc)){
                    min_cyc = low_k;
                }
            }
        }
    ...
    cycdata.second = min_cyc;
    return cycdata;
}

void cycle_minimize_full(vector<vector<int>>& nullspace){
	int min_cyc_len = 3; pair<int,int> cycdata; int high_size, previous_high_size;
	cycdata = Cycle_minimize(nullspace, min_cyc_len);
	high_size = cycdata.first; min_cyc_len = cycdata.second; previous_high_size = high_size+1;
	while((high_size > 0)&(high_size != previous_high_size)){
		previous_high_size = high_size;
		cycdata= Cycle_minimize(nullspace, min_cyc_len);
